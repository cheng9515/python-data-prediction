...
class gru():
    ...
    def gru(self):
        ...
        # Create and fit the gru network/创建并拟合LSTM网络

        gru_model = Sequential()
        for i in range(len(self.num_neur)):  # 构建多层网络
            if len(self.num_neur) == 1:
                gru_model .add(GRU(self.num_neur[i], input_shape=(None, self.look_back)))
            else:
                if i < len(self.num_neur) - 1:
                    gru_model.add(GRU(self.num_neur[i], input_shape=(None, self.look_back), return_sequences=True))
                else:
                    gru_model.add(GRU(self.num_neur[i], input_shape=(None, self.look_back)))
        gru_model .add(Dense(1)) #dense就是全连接层
        gru_model .summary()  # Summary the structure of neural network/网络结构总结
        gru_model .compile(loss='mean_squared_error', optimizer='adam')  # Compile the neural network/编译网络
        gru_model .fit(self.x_train, self.y_train, epochs=self.epochs, batch_size=self.batch_size
                       , verbose=0)  # Fit the GRU network/拟合GRU网络
        ...
        # GRU prediction/GRRU进行预测
        trainPredict = gru_model.predict(self.x_train)  # Predict by training data set/训练集预测
        testPredict = gru_model .predict(self.x_test)  # Predict by test data set/测试集预测
    ...
    def plot(self, scaler, trainPredict, testPredict):
        ...
        dates = pd.date_range('2001-06', periods=len(y), freq='M')
        ...
        plt.axvline(xs[44], linewidth='2', color='black')  # 画直线区分训练部分与测试部分
        ...
        plt.savefig(r'D:\pycharmfile\pydatafile\lstm_india.png', dpi=900)  # 保存图片
...
if __name__ == "__main__":
    ...
    file = r'D:\pycharmfile\pydatafile\india_data.xlsx'
    dataframe = pd.read_excel(file, sheet_name=0, header=0, index_col=None)
    dataset = dataframe.iloc[:, [1, 2, 3, 4, 5, 6]].values
...
#    dataset = scaler.fit_transform(dataset)
    ...
    batch_size = 44 # Batch size/批数大小
    select_feature = [1, 1, 1, 1, 1, 1]  # Selected features list/被选择特征列表
    ...
    model = gru(dataset, hyper_params)  # Create instance of LSTM/实例化模型
#    saver = tf.train.Saver()
#    with tf.Session() as sess:
#        sess.run(tf.global_variables_initializer())
#        saver.save(sess,"D:\pycharmfile\pyfile\lstm_india.ckpt")
    trainPredict, testPredict, y_train, y_test = model.gru()  # Create and fit the LSTM network/创建并拟合LSTM网络
    trainMAPE, testMAPE, trainPredict, testPredict = model.mape(dataset, trainPredict
                                                                , testPredict)  # Evaluate network performance/评估网络效果
#    model.plot(dataset, trainPredict, testPredict)  # Visualization results/可视化结果
    print(trainPredict)
    print(testPredict)
