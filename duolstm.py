...
from bayes_opt import BayesianOptimization #加入贝叶斯优化调参
...
class lstm():
    ...
    def plot(self, scaler, trainPredict, testPredict):
        ...
        dates = pd.date_range('2001-11', periods=len(y), freq='M')
        ...
        plt.axvline(xs[171], linewidth='2', color='black')  # 画直线区分训练部分与测试部分
        ...
        plt.savefig(r'D:\pycharmfile\pydatafile\lstm149.png', dpi=900)  # 保存图片
...
if __name__ == "__main__":
    ...
    file = r'D:\pycharmfile\pydatafile\M_JH050.xlsx'
    dataframe = pd.read_excel(file, sheet_name=0, header=0, index_col=None)
    dataset = dataframe.iloc[:, [1, 2, 3, 4, 5]].values
    ...
    epochs = 3000 # Training times/训练次数
    batch_size = 1 # Batch size/批数大小
    select_feature = [1, 1, 1, 1, 1]  # Selected features list/被选择特征列表
